...
const checkDesc = check("desc", "Please provide the T-Shirt description")
  .not()
  .isEmpty();
const checkPrice = check("price", "Please enter the T-Shirt price").isNumeric();
const checkQuantity = check(
  "quantity",
  "Please enter the number of T-Shirts"
).isNumeric();
router.post(
  "/",
  [
    upload.array("shirt_image", 4),
    [checkTitle, checkDesc, checkPrice, checkQuantity],
    auth,
    ROLE("ADMIN"),
  ],
  async (req, res) => {
...
router.patch("/:id", [auth, ROLE("ADMIN")], async (req, res) => {
  ...
  try {
    const tShirt = await Product.findByIdAndUpdate(
      id,
      { $set: req.body },
      { new: true }
    );
...
router.delete("/", [auth, ROLE("ADMIN")], async (req, res) => {
  ...
  try {
    ...
    if (!tshirt) {
      return res.status(400).json({
        errors: [{ msg: `Something went wrong, tshirt not deleted` }],
      });
    }
